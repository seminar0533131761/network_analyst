...
from my_modules.module_authentication import fake_users_db
...
async def login_for_access_token(response: Response, form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    user = await module_authentication.authenticate_user(fake_users_db, form_data.username, form_data.password)
    if not user:
        raise HTTPException(
